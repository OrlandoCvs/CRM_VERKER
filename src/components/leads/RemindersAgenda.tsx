...
const BUCKET_META: Record<Bucket, { label: string; tone: string }> = {
  ...
  upcoming: { label: 'Próximos', tone: 'text-gray-500' },
}
            ...
            return (
              <div key={bucket}>
                <h3 className={`mb-2 flex items-center gap-2 text-sm font-semibold ${meta.tone}`}>
                  {meta.label}
                  <span className="rounded-full bg-gray-100 px-1.5 text-xs font-medium text-gray-500">
                    {items.length}
                  </span>
                </h3>
...
function AgendaRow({
  ...
  return (
    <div className="group flex items-center gap-3 px-4 py-3 transition-colors hover:bg-gray-50">
      <button
        onClick={() => onComplete(reminder.id)}
        className="flex h-5 w-5 shrink-0 items-center justify-center rounded-md border border-gray-300 text-transparent transition-colors hover:border-emerald-400 hover:text-emerald-500"
      ...
      <Link
        ...
        className="flex shrink-0 items-center gap-1.5 rounded-lg px-2 py-1 text-xs text-gray-500 transition-colors hover:bg-blue-50 hover:text-blue-600"
      >
